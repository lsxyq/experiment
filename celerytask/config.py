...
CELERYBEAT_SCHEDULE = {
    'add': {
        'task': 'celerytask.tasks.add',  # 运行定时任务test_celery, 要写路径中的tasks.py，必须为tasks.py文件
        'schedule': timedelta(seconds=3),  # 设置循环时间
        'args': (19, 16)  # 给定时任务的参数
    },
    'test': {
        'task': 'celerytask.tasks.test',  # 运行定时任务test_celery, 要写路径中的tasks.py，必须为tasks.py文件
        'schedule': timedelta(seconds=3),  # 设置循环时间
